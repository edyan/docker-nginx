...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Run (with PHP) :"
    echo "  docker run -d --rm --name php-test-ctn edyan/php:latest"
    echo "  docker run -d --rm -e PHP_HOST=php-test-ctn --name nginx${VERSION}-test-ctn edyan_nginx${VERSION}_test"
